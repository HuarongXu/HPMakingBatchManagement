...
| 页面 | 路由 | 功能 |
|---|---|---|
| Dashboard 总览 | `/` | KPI 卡片、系统产能热力图 (by Day + by Shift 双图联动)、告警预览（点击 View 跳转并自动筛选） |
| Orders 订单明细 | `/orders` | 全量订单表格，支持搜索/筛选/排序/分页/导出/详情面板/Batch Count 列/Batch Note 列/单元格计算器(Ctrl+Click求和)/告警跳转URL筛选 |
