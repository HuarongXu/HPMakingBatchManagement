...
|---|---|---|
| 4.4 | 4.28 ~ 4.52 | 4.10 ~ 4.70 |
| 8.8 | 8.62 ~ 8.98 | 8.30 ~ 9.30 |
| 11.0 | 10.76 ~ 11.24 | 10.30 ~ 11.70 |
| 13.2 | 12.90 ~ 13.50 | 12.30 ~ 14.10 |
...
4. 至少一侧为“欠装单批”（即单订单批次且 `Load / Target < 0.90`）；
...
- 合并完成后按订单的 `start_datetime + shift` 重新分摊 `batch_count`；
...
2. **防止连环放大**：仅允许“修复型合并”（至少一侧欠装单批），避免把已健康批次持续向上并到过大目标。
...
> 当前文档基于 2026-04-10 代码版本整理，覆盖双层容差、日期班次容量、多订单搭批组合、大目标优先搭批策略、真实批次汇总、单单备注策略与可解释决策输出。